...
class RoverBrain2(Rover):
    def __init__(self, driver, save):
        ...
        self.D_2 = torch.randn(self.ps2**2, self.k2).float().cuda(0)
        self.num_rows, self.num_cols = self.imsz // self.downsample
        ...
        self.save_dict = save
        self.rec = torch.zeros(self.k, self.num_rows, self.num_cols)
        self.run()
    ...
    def X3(self, x, D, D_2):
        ...
        x = x.unfold(0, self.ps, self.downsample).unfold(1, self.ps, self.downsample).unfold(2, 3, self.downsample)
        ...
        for i in range(self.k):
            self.rec[i, ...] = aa[i, :].contiguous().view(self.num_rows,
                                                          self.num_cols)
        self.rec = self.rec.unfold(0, 1, 1).unfold(1, self.ps2, 7)
        self.rec = self.rec.unfold(2, self.ps2, 7).cuda(0)
        n, h, w, _, _, _ = self.rec.size()
        self.rec = torch.t(self.rec.contiguous().view(-1, self.ps2**2))

        D_2 = torch.mm(D_2,
                       torch.diag(1./(torch.sqrt(torch.sum(D_2**2, 0))+e)))
        x2 = self.whiten(self.rec - torch.mean(self.rec, 1)[:, None])
